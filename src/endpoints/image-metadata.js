...
export async function initializeMetadataForDirectory(userDataRoot, subDirectory, type) {
    ...
    const { results, generatedCount } = await getOrGenerateMetadataBatch(
        userDataRoot,
        relativePaths,
        type,
    );
    ...
    if (generatedCount > 0) {
        console.log(
            `[ImageMetadata] Generated metadata for ${generatedCount} new images in ${subDirectory}`,
        );
    }
...
export async function initializeAllUserMetadata(userDirectories) {
    try {
        ...
        console.error(
            '[ImageMetadata] Failed to initialize background metadata:',
            error.message,
        );
    }
}
